...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  /* For each of the following modifiers, use what you learned about modifiers
   ...
   Note that the uninitialized Item. is 0, which is also the index of the ForSale value,
   so checking that Item.State == ForSale is not sufficient to check that an Item is for sale.
   Hint: What item properties will be non-zero when an Item has been added?
   */
  modifier forSale(uint sku){ require( (items[sku].state==State.ForSale) && items[sku].buyer == address(0) ); _;}
  ...
  function buyItem(uint sku) forSale(sku) paidEnough(items[sku].price) checkValue(sku)
    public payable
  {
        // This  should transfer money to the seller,
